...
class ClangArgs(private val configurables: Configurables) : Configurables by configurables {
    ...
    companion object {
        @JvmStatic
        fun filterGradleNativeSoftwareFlags(projectDir:String, args: MutableList<String>) {
            args.remove("/usr/include") // HACK: over gradle-4.4.
            when (PlatformManager(Distribution(konanHomeOverride = "${projectDir}")).hostPlatform.clang.host) {
